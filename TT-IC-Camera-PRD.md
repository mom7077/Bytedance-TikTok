...
## 1. 背景与目标
- 背景：移动端日常分享/创作需要轻量、高效、流畅的图片编辑工具，覆盖相册选图、拍照、基础编辑、装饰、保存。
- 业务目标：完成必做功能并流畅运行，满足基础编辑需求；在 3 分钟内可演示核心流程。
- 技术目标：掌握相册/相机接入、图像加载与处理、保存与权限处理、性能优化；Android 10+ 兼容。
- 成功标准：启动 ≤1s；导入/拍照到编辑 ≤1s；调节预览响应 ≤0.5s；编辑 10 张无闪退；内存 ≤200MB；保存成功率 >99%，异常有明确提示。

## 2. 用户与场景
- 目标用户：普通内容发布者、社交分享用户、轻量创作者。
- 主要场景：从相册快速选图并裁剪/调节/加文字后保存；现场拍照→快速加文字→保存。
...
- 保存：写存储权限；失败提示（权限/空间不足）；保存成功提示并入相册；自动加水印“训练营”（默认右下留边距），仅针对图片。
- 实现约束：仅 Android 端（Android 10+），不含 iOS；相册/编辑/合成等核心逻辑自研实现，第三方仅用于辅助（如权限、图片加载），使用需在文档说明理由与范围。
...
- 大图预览 → 编辑页。
- 编辑页：主画布居中；底部工具栏（裁剪、旋转/翻转、调节、文字、更多）；顶部：返回、撤销/重做、保存。
...
## 7. 性能与非功能
- 启动 ≤1s；相册滑动帧率 ≥30fps。
- 选图/拍照至编辑 ≤1s；调节参数预览响应 ≤0.5s。
- 内存占用：编辑 10MB 内单图时 ≤200MB；避免泄漏。
- 稳定性：连续编辑 10 张无闪退。
- 兼容性：仅 Android 10+，至少两款真机测试；相册资源量大时仍需保持启动时延与滑动帧率指标。

## 8. 权限与异常
- 存储/相机权限分场景请求，拒绝后给说明与再授权入口。
- 异常态：无权限、无图片、无相机、存储空间不足、保存失败均有提示与下一步指引。

## 9. 数据埋点（可选）
- 启动耗时、相册加载耗时、功能使用频次（裁剪/旋转/调节/文字/保存）、保存成功率、异常分布。

## 10. 里程碑（示例）
- W1：相册（全部/文件夹、空态、权限指引）/相机直达编辑、大图预览单选、基础手势缩放平移。
- W2：裁剪、旋转/翻转、亮度对比度实时预览与重置、文字层（编辑/拖拽/缩放/旋转）、撤销/重做。
- W3：保存与水印合成、权限/空间不足等异常提示、稳定性与性能验证（10 张连续编辑、帧率/内存）。
- W4：适配优化、埋点接入、文档与演示视频。
...
## 12. 交付物与评审
- 交付物：GitHub 仓库代码及 README、技术文档（含关键模块类图/时序图）、不超过 3 分钟的演示视频、学习总结。
- 评审参考：功能完整性 40%、性能与稳定性 20%、界面与交互 20%、代码质量 20%。
